...
def save_analysis(user_id, filename, career, skills, roadmap, courses, raw_output=""):
    conn   = _get_conn()
    cursor = conn.cursor()
    try:
        cursor.execute(
            ...
            (user_id, filename, career,
             json.dumps(skills), roadmap, json.dumps(courses), raw_output)
        )
        conn.commit()
        return cursor.lastrowid
    except Exception as e:
...
def get_user_history(user_id: int, limit: int = 20) -> list:
    ...
    for row in rows:
        ...
        row["created_at"] = row["created_at"].strftime("%d %b %Y, %I:%M %p") if row["created_at"] else ""
    return rows
